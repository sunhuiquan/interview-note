...
今年行情太难加上以后想干数据库方向的工作，不打算刷题背面经花两三个月时间冲秋招了，打算利用接下来半年好好学 DB，转正失败的话就等明年春招再投，春招也不行就攒钱出国读个一年水硕润，寄。
...
C++后端开发实习生  
1. DolphinDB Python Parser 支持 Pandas 和 Numpy 库
2. 尝试对每一个 Python Session 线程产生一个进程，通过使用子进程的 GIL，解决多线程争抢 GIL 的问题（尝试ing）
3. TODO
